...
<body style="padding-top:100px;" ng-controller="ControladorPrincipal">
    ...
    <script>
    angular.module('blog', ['ui.bootstrap', 'datatables', 'xeditable']);
    ...
    function ControladorPrincipal($scope, $compile, $http, $q, uibModal, DTOptionsBuilder, DTColumnDefBuilder) {
        ...
        $scope.showTelefono = function(user) {
            var selected = [];
            if (user.telefono) {
                selected = $filter('filter')($scope.telefono, {
                    value: user.telefono
                });
            }
        ...
        $scope.saveColumn = function(column) {
            ...
            angular.forEach($scope.users, function(user) {
                results.push($http.post('/saveColumn', {
                    column: column,
                    value: user[column],
                    id: user.id
                }));
